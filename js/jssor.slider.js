...
new function () {
    ...
    function GetEasing(transition) {
        ...
        if (!cache) {
            ...
            //if ($JssorUtils$.$IsBrowserBadTransform()) {
            //    cache.$MarginTop = cache.$Top;
            //    cache.$MarginLeft = cache.$Left;
            //}
        }
                    ...
                    var col = formationItem[1];
                    {
                        ...
                        if (slideTransition.$Zoom || slideTransition.$Rotate) {
                            ...
                            if (allowRotate) {
                                ...
                                else if ($JssorUtils$.$IsBrowserFireFox())
                                    _StyleEnd.$Zoom = Math.min(_StyleEnd.$Zoom, 3);
    ...
    $JssorSlideshowRunner$ = window.$JssorSlideshowRunner$ = function (slideContainer, prevItem, slideItem, transition, slideContainerWidth, slideContainerHeight, loadingContainer, slideshowOptions, slideStoppedCallback) {

        transition = /*_PrototypeTransitions[transition.$Index] = _PrototypeTransitions[transition.$Index] || */EnsureTransitionInstance(transition);
        ...
        var _LiveStatus = true;
        var _FreezStatus = false;
        var _CancelStatus = false;
        ...
        function Initialize() {
            var prevSlideElement = prevItem ? prevItem.$Item : loadingContainer;
            var currentSlideElement = slideItem.$Item;
            prevSlideElement["no-image"] = !prevItem || !prevItem.$Image;
            currentSlideElement["no-image"] = !slideItem.$Image;

            var slideElementAbove = prevSlideElement;
            var slideElementBelow = currentSlideElement;

            var slideTransitionAbove = transition;
            var slideTransitionBelow = transition.$Brother || EnsureTransitionInstance({});
        ...
        function ShowFrame() {
            ...
            if (_LiveStatus && !_FreezStatus) {
                var time = $JssorUtils$.$GetNow() - _SliderStartTime;

                _LiveStatus = (time < _EndTime);

                _SlideshowPlayerAbove.$ShowFrame(time);
                _SlideshowPlayerBelow.$ShowFrame(time);
            }
        ...
        function Play(interval) {
            function PlayFrame() {
                if (!_CancelStatus) {
                    if (_FreezStatus || ShowFrame()) {
                        _SliderAnimationTimeoutHandler = $JssorUtils$.$Delay(PlayFrame, interval);
                    }
                    else {
                        StopInternal();
            ...
            var _SliderAnimationTimeoutHandler = $JssorUtils$.$Delay(PlayFrame, interval);
        }
        ...
        $JssorDebug$.$LiveStamp(self, "slideshow_runner_" + _SlideshowRunnerCount);
    };
    //JssorSlideshowRunner--------
    ...
    function JssorSlider(elmt, options) {
        ...
        function GetTransition() {
            ...
            if (_TransitionsOrder) { /*Sequence*/
                var transitionCount = _Transitions.length;
                if (transitionCount > _SlideCount && ($JssorUtils$.$IsBrowserChrome() || $JssorUtils$.$IsBrowserSafari() || $JssorUtils$.$IsBrowserFireFox())) {
                    transitionCount -= transitionCount % _SlideCount;
                }
            ...
            _Transitions[n] && (_Transitions[n].$Index = n);
        ...
        function SlideItem(slideBoard, slideElmt, index) {
            ...
            function LoadImageCompleteEventHandler(completeCallback, image) {
                _ImageLoaded = true;
            ...
            function DragStartEventHandler() {
                _IsCaptionSliderPlayingWhenDragStart = _CaptionSliderCurrent && _CaptionSliderCurrent.$IsPlaying();
                _SelfSlideItem.$StopCaptionSlider();
            }
            ...
            function ResetCaptionSlider() {
                ...
                _CaptionSliderCurrent = _CaptionSliderIn;
            ...
            _SelfSlideItem.$UncaptureLinkClick = function (linkClickEventHandler) {
                $JssorUtils$.$Each(_ChildElements, function (childElement, i) {
                    $JssorUtils$.$RemoveEvent(childElement, "click", linkClickEventHandler);
                });
            };

            _SelfSlideItem.$CaptureLinkClick = function (linkClickEventHandler) {
                $JssorUtils$.$Each(_ChildElements, function (childElement, i) {
                    $JssorUtils$.$SetStyleZIndex(childElement, $JssorUtils$.$GetStyleZIndex(childElement) + 1);
                    $JssorUtils$.$AddEvent(childElement, "click", linkClickEventHandler);
                });
            };
            ...
            _SelfSlideItem.$StampSlideItemElements = function (stamp) {
                ...
                $JssorDebug$.$Execute(function () {
                    $JssorUtils$.$SetAttribute(loadingContainer, "debug-id", stamp + "_loading_container_id");
                });
            };
            ...
            _SelfSlideItem.$StopCaptionSlider = function () {
                _CaptionSliderCurrent && _CaptionSliderCurrent.$Stop();
            };

            _SelfSlideItem.$OnBeforeIn = function (callback, beforeSlideshow) {
            };
            ...
            _SelfSlideItem.$OnAfterIn = function (callback, afterSlideshow) {
                ...
                var currentCaptionSlider = _CaptionSliderCurrent = _CaptionSliderCurrent || ResetCaptionSlider();

                if (currentCaptionSlider.$Version < _CaptionSliderOptions.$Version && !currentCaptionSlider.$IsOnTheWay()) {
                    currentCaptionSlider = ResetCaptionSlider();
                }

                currentCaptionSlider.$PlayToEnd(callback);
            };

            _SelfSlideItem.$OnBeforeOut = function (callback, beforeSlideshow) {
                if ((_CaptionSliderOptions.$PlayOutMode & 1) && !(_CaptionSliderOptions.$PlayOutMode & 4)) {
                    _CaptionSliderOut.$GoToEnd();
                    _CaptionSliderOut.$PlayToBegin(callback);
                    _CaptionSliderCurrent = _CaptionSliderOut;
                }
                else {
                    callback && callback();
                }
            ...
            _SelfSlideItem.$OnAfterOut = function (callback, afterSlideshow) {
                _SelfSlideItem.$StopCaptionSlider();
            ...
            //SlideItem Constructor
            {
                ...
                _ChildElements = $JssorUtils$.$FindChildrenByTag(slideElmt, null, true);

                _ImageItem = $JssorUtils$.$FindFirstChildByAttribute(slideElmt, "image");
                ...
                slideBoard.$On(21, ParkEventHandler);
                slideBoard.$On(22, DragStartEventHandler);
                ...
                $JssorDebug$.$Execute(function () {
                    $JssorUtils$.$SetAttribute(_Wrapper, "debug-id", "slide-" + index);
                });
            }
        }
        ...
        //SlideBoard
        function SlideBoard() {
            var _SelfSlideBoard = this;
            $JssorEventManager$.call(_SelfSlideBoard);
            ...
            function OnDragStart(event) {
                ...
                _SelfSlideBoard.$TriggerEvent(22);
            }
            ...
            function OnPark(index, slideItem, prevIndex, afterSlideshow) {
                _DragOrientation = 0;
                if (IsCurrentSlideIndex(index) && _SlideshowRunner && _IsSlideshowFrozen) {
                ...
                if (!IsCurrentSlideIndex(prevIndex))
                    _SlideItems[prevIndex] && _SlideItems[prevIndex].$OnAfterOut();

                slideItem.$OnAfterIn($JssorUtils$.$CreateCallback(null, OnSlideAfterIn, index), afterSlideshow);

                //Trigger EVENT_PARK
                _SelfSlideBoard.$TriggerEvent(21, index, prevIndex);
            }

            function OnSlideAfterIn(index) {
                //Trigger EVENT_CAPTION_IN
                self.$TriggerEvent(201, index);
                ResetTimeToGo();
            ...
            function GoForNextSlide() {
                ...
                if (indexToGo == -1)
                    indexToGo = _Options.$StartIndex || 0;
                else
                    indexToGo += 1;

                if (_SlideshowEnabled && _AutoPlay) {
                    _IsSlideshowFrozen = false;
                    _IsSlideshowRunning = 1;

                    _Slideshow.$GoForSlideshow(GetRealIndex(indexToGo));
                }
                else {
                    if (_CurrentSlideIndex != -1)
                        indexToGo += _AutoPlaySteps - 1;
            ...
            function Run() {
                if (!_IsDragging && !_IsSliderRunning && (_AutoPlay && _IsActive && !_IsPreparingToGo && !_IsRequesting && (!_IsSlideshowRunning || _IsSlideshowFrozen) || _CurrentSlideIndex == -1)) {
                    _TimeToGo -= 60;
            ...
            function PrepareRun() {
                ...
                if (_SlideshowEnabled || document.readyState === 'complete')
                    Run();
                else
                    $JssorUtils$.$Delay(PrepareRun, 30);
            }
            ...
            function Slideshow() {
                ...
                //                _Wrapper.style.filter = "inherit";
                //                _Wrapper.style.zoom = 1;
                var _SlideItem;
                var _CurrentLink;

                $JssorAnimator$.call(_Self, -1, 2, { $Easing: $JssorEasing$.$EaseLinear, $Setter: { $Position: SetPosition} }, _Wrapper, { $Position: 1 }, { $Position: -1 });

                function RunSlideshow(index, slideItem, prevIndex, prevItem, transition) {
                    if (_AutoPlay && _IsActive && !_IsDragging && !_IsSliderRunning && !_IsRequesting) {

                        var onSlideshowStoppedCallback = $JssorUtils$.$CreateCallback(null, OnSlideshowStoppedCallback, index, slideItem, prevIndex);

                        _SlideshowRunner = new _SlideshowRunnerClass(_SlideContainer, prevItem, slideItem, transition, _SlideWidth, _SlideHeight, _LoadingContainer, _SlideshowOptions, onSlideshowStoppedCallback);
                ...
                function OnSlideshowStoppedCallback(index, slideItem, prevIndex) {

                    //var slideshowStoppedClosure = {};
                    //$JssorDebug$.$LiveStamp(slideshowStoppedClosure, "slideshow_stopped_closure_" + _SlideshowRunnerCount);

                    _Self.$Clear();
                    OnPark(index, slideItem, prevIndex, true);
                }
                ...
                _Self.$GoForSlideshow = function (index) {
                    ...
                    if (!transition)
                        return GoToLocal(index);

                    _SlideItem = _SlideItems[index];

                    _SlideItem.$OnBeforeIn();
                    ...
                    var prevIndex = _CurrentSlideIndex;
                    var prevItem = _PrevSlideItem;
                    ...
                    _SlideItem.$LoadImage($JssorUtils$.$CreateCallback(null, RunSlideshow, index, _SlideItem, prevIndex, prevItem, transition));
                };
                ...
                _Self.$Clear = function () {
                    if (_SlideshowRunner) {
                        ...
                        _SlideContainer.$Clear();
                        _SlideshowRunner = null;
                    }
                };
                ...
                //Slideshow Constructor
                {
                    ...
                    $JssorDebug$.$Execute(function () {
                        ...
                        //$JssorUtils$.$SetAttribute(_SlideContainer2.$Elmt, "debug-id", "slide_container2");
                    });

                    //$JssorUtils$.$AppendChild(_Wrapper, _SlideContainer.$Elmt);
            ...
            function CarouselPlayer(carousel, slideshow) {
                ...
                _Self.$OnStop = function () {
                    ...
                    if (currentSlideInfo.$Position == 0) {
                        OnPark(currentSlideInfo.$Index, _SlideItems[currentSlideInfo.$Index], _CurrentSlideIndex);
                    }
                };
                ...
                _Self.$PlayCarousel = function (fromPosition, toPosition, duration, callback) {
                    ...
                    _SlideItems[_CurrentSlideIndex] && _SlideItems[_CurrentSlideIndex].$StopCaptionSlider();
            ...
            _SelfSlideBoard.$GoToLocal = function (index, callback, specifiedDuration) {
            ...
            //_SelfSlideBoard.$StopSlideshow = function () {
            //    _Slideshow.$StopSlideshow();
            //};

            _SelfSlideBoard.$PlaySlideshow = function () {
                ResetTimeToGo(1);
            };

            _SelfSlideBoard.$SlideItems = _SlideItems;

            _SelfSlideBoard.$Run = function () {
            ...
            //SlideBoard Constructor
            {
                ...
                for (var i = 0; i < _SlideElmts.length; i++) {
                    ...
                    var slideItem = new SlideItem(_SelfSlideBoard, slideElmt, i);
                ...
                $JssorDebug$.$Execute(function () {
                    if (_LoadingContainer)
                        $JssorUtils$.$SetAttribute(_LoadingContainer, "debug-id", "theloadingcontainer");
                });
        ...
        function UnregsterDrag() {
            JssorSlider.$DragRegistry &= ~_DragOrientationRegistered;
        }
        ...
        function CreatePanel() {
            ...
            $JssorUtils$.$SetStylePosition(div, "relative");
        ...
        function GoTo(index, callback, specifiedDuration) {
            _SlideBoard.$GoToLocal(index, callback, specifiedDuration);
        }
        ...
        function ParkEventHandler(currentIndex, fromIndex) {
            self.$TriggerEvent(21, currentIndex, fromIndex);
        }
        ...
        self.$SetScaleWidth = function (width) {
            ...
            if (!_ScaleWrapper) {
                ...
                //$JssorUtils$.$SetStyleZIndex(_ScaleWrapper, 0);
            ...
            $JssorDebug$.$Execute(function () {
                if (!initialScrollWidth) {
                    initialScrollWidth = self.$Elmt.scrollWidth;
                }
            });
            ...
            //        var text = "windowWidth: " + $JssorUtils$.$GetWindowSize(window).x + " initialScrollWidth: " + initialScrollWidth + " scrollWidth: " + self.$Elmt.scrollWidth + " scaleWidth: " + self.$GetScaleWidth() + " devicePixelRatio: " + window.devicePixelRatio;

            //        $JssorUtils$.$SetInnerText(debugInfoElement, text);
            //    }
            //});
        };
        ...
        //$JssorSlider$ Constructor
        {
            ...
            if (_SlideshowOptions) {
                ...
                $JssorDebug$.$Execute(function () {
                    if (!_Transitions || !_Transitions.length) {
            ...
            _SlideBoard.$On(21, ParkEventHandler);
            //_SlideBoard.$On(22, DragStartEventHandler);
            ...
            self.$SetScaleWidth(self.$GetOriginalWidth());
            _SlideBoard.$Run();
        }
    }
    ...
    JssorSlider.$EVENT_PARK = 21;
    //JssorSlider.$EVENT_DRAGSTART = 22;
    JssorSlider.$EVENT_CAPTION_IN = 201;
...
var $JssorDirectionNavigator$ = window.$JssorDirectionNavigator$ = function (elmt, options, uiSearchMode) {
    ...
    function OnNavigationRequest(steps) {
        self.$TriggerEvent($JssorNavigatorEvents$.$NAVIGATIONREQUEST, _CurrentIndex + steps);
    }
        ...
        if (!_Initialized) {
            ...
            $JssorDebug$.$Execute(function () {
                if (!_Options.$AutoCenter) {
                    ...
                    if (isNaN(slidesContainerTop)) {
                        $JssorDebug$.$Fail("Position 'top' wrong specification of thumbnail navigator slides container (<div u=\"thumbnavigator\">...<div u=\"slides\">), \r\nwhen option $ThumbnailNavigatorOptions.$AutoCenter set to 0, it should be specied by inline style in pixels (like <div u=\"slides\" style=\"top: 0px;\">)");
                    }

                    if (isNaN(slidesContainerLeft)) {
                        $JssorDebug$.$Fail("Position 'left' wrong specification of thumbnail navigator slides container (<div u=\"thumbnavigator\">...<div u=\"slides\">), \r\nwhen option $ThumbnailNavigatorOptions.$AutoCenter set to 0, it should be specied by inline style in pixels (like <div u=\"slides\" style=\"left: 0px;\">)");
                    }
                }
            });
            ...
            $JssorUtils$.$Each(items, function (item, index) {
                ...
                var columnIndex = index % _Count;
                var laneIndex = Math.floor(index / _Count);
    ...
    function CreateAnimator(item, transition, immediateOut) {
        ...
        if (transition.$Zoom || transition.$Rotate) {
            ...
            if ($JssorUtils$.$IsBrowserIe9Earlier())
                ...
                toStyles.$Zoom = Math.min(toStyles.$Zoom, 3);
