...
// Added by Pushpal on 16/12/2012 07:23 PM to grab and add remote images to Ads
function process($file) {
		$rows = $this->app_csv_to_array($file);
		foreach($rows as $row) {
			$post['post_title'] = sanitize_text_field($row['post_title']);
		...
		foreach($row as $key => $val) {
			if(strstr($key, 'cp_')) {
				add_post_meta($post_id, $key, sanitize_text_field($val), true);
				if($row['cp_images'] != '') {
					if(strpos($row['cp_images'], ',')) {
						$remote_img = str_replace(', ', ',', $row['cp_images']);
						$cp_img_arr = explode(",", $remote_img);
						foreach($cp_img_arr as $each_cp_img) {
							$img_name = substr(strrchr($each_cp_img, "/"), 1);
							$img = imagecreatefromjpeg($each_cp_img);
							$year = date('Y');
							$month = date('m');
							$path = $_SERVER['DOCUMENT_ROOT']."/classifieds/wp-content/uploads/".$year.'/'.$month.'/'.$img_name;
							imagejpeg($img, $path);
						...
						$remote_img = $row['cp_images'];
						$img_name = substr(strrchr($remote_img, "/"), 1);
