...
function rb_wptouch_core_header_home() {
	if (bnc_is_home_enabled()) {
		echo sprintf(__( "%sHome%s", "wptouch" ), '<li><a href="' . get_bloginfo( 'url' ) . '"><img src="' . compat_get_plugin_url( 'wptouch' ) . '/themes/default/images/ico_home.png" alt="" />','</a></li>');
	}
}
  
function rb_wptouch_core_header_pages() {
...
function rb_wptouch_core_header_rss() {
	if (bnc_is_rss_enabled()) {
		echo sprintf(__( "%sRSS Feed%s", "wptouch" ), '<li><a href="' . get_bloginfo('rss2_url') . '?post_type=ad_listing"><img src="' . compat_get_plugin_url( 'wptouch' ) . '/themes/default/images/ico_rss.png" alt="" />','</a></li>');
	}
}

function rb_wptouch_core_header_email() {
	if (bnc_is_email_enabled()) {
		echo sprintf(__( "%sE-Mail%s", "wptouch" ), '<li><a href="mailto:' . get_bloginfo('admin_email') . '"><img src="' . compat_get_plugin_url( 'wptouch' ) . '/themes/default/images/ico_mail" alt="" />','</a></li>');
	}
}
// rubencio ends
...
function cp_get_ad_details($postid, $catid, $locationOption = 'list') 
{
        ...
        if($results) {
                    ...
                    foreach ($results as $result) :
                        ...
                        if (!empty($post_meta_val))
                            if($result->field_type == "checkbox")
								echo '<li id="'. $result->field_name .'"><span>' . $result->field_label . ':</span> ' . str_replace(',', ', ', cp_make_clickable($post_meta_val)) .'</li>'; // make_clickable is a WP function that auto hyperlinks urls}
							elseif($result->field_name != 'cp_price' && $result->field_type != "text area")
                                echo '<li id="'. $result->field_name .'"><span>' . $result->field_label . ':</span> ' . cp_make_clickable($post_meta_val) .'</li>'; // make_clickable is a WP function that auto hyperlinks urls
                    ...
                    foreach ($results as $result) :
                        ...
                        if (!empty($post_meta_val))
                            if($result->field_name != 'cp_price' && $result->field_type == 'text area')
                                echo '<div id="'. $result->field_name .'" class="custom-text-area dotted"><h3>' . $result->field_label . '</h3>' . cp_make_clickable($post_meta_val) .'</div>'; // make_clickable is a WP function that auto hyperlinks urls
          ...
          echo __('No ad details found.', 'classipro');
