...
<?php 
 ...
 ?>
    ...
    <?php } ?>
			...
			<?php if ( ! is_single() && is_sticky() && $wptouch_settings['post-cal-thumb'] == 'nothing-shown' ) echo '<div class="sticky-icon-none"></div>'; ?>
			<?php if ( ! is_single() && is_sticky() && $wptouch_settings['post-cal-thumb'] != 'nothing-shown' ) echo '<div class="sticky-icon"></div>'; ?>
	...
	<a class="h2" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
		...
		<?php if ($wptouch_settings['post-cal-thumb'] != 'calendar-icons') { ?><?php echo get_the_time('d.m.Y') ?> <?php if (!bnc_show_author()) { echo '<br />';} ?><?php } ?>
			<?php if (bnc_show_author()) { ?><span class="lead"><?php _e("by", "wptouch"); ?></span> <?php the_author(); ?><br /><?php } ?>
			<?php if (bnc_show_categories()) { if ( get_the_category() ) the_category(', '); else echo get_the_term_list( $post->ID, APP_TAX_CAT, '', ', ', '' ); echo('<br />'); } ?> 
            ...
            <div id="entry-<?php the_ID(); ?>" <?php  if (bnc_excerpt_enabled()) { ?>style="display:none"<?php } ?> class="mainentry <?php echo $wptouch_settings['style-text-justify']; ?>">
 				<?php the_excerpt(); ?>
			<a class="read-more" href="<?php the_permalink() ?>"><?php _e( "See Ad Details", "wptouch" ); ?></a>
	        </div>  
      </div><!-- rb_inside -->
      </div><!-- post -->
