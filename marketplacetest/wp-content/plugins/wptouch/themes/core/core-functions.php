...
function bnc_get_ordered_cat_list( $num ) 
{
	...
	$cats = get_terms( "ad_cat", array( "hide_empty" => 0, "hierarchical" => 1, "show_count"=>1, "pad_counts"=>1, "exclude" => $exclude_cats)  );
	...
	foreach ($cats as $c)
	{
		if ($c && sizeof($c) > 0) 
		{
				...
				$subcats = get_terms($c->taxonomy, array( "hide_empty" => 0, "pad_counts"=>1, "hierarchical"=>1 ,"show_count"=>0,"child_of" => $c->term_id, "exclude" => $exclude_cats ));		
...
function wptouch_core_body_result_text() {  
	...
	{	
		if (is_search()) 
		{
			echo sprintf( __("Search results for '%s'", "wptouch"), get_search_query() );
		}
		elseif (is_category()) 
		{
		...
		{
			 echo sprintf( __("Category: %s", "wptouch"), single_cat_title("", false));
		}
		elseif (is_tax('ad_tag'))
		{
			 echo sprintf( __("Tag:  %s", "wptouch"), single_tag_title("", false));
		}
	}
}
