/*
 * cpmobile 1.9.x -The cpmobile Core JS File
 */
...
$wpt.fn.cpmobileFadeToggle = function( speed, easing, callback ) { 
	return this.animate( {opacity: 'toggle'}, speed, easing, callback ); 
};

function cpmobile_switch_confirmation( e ) {
	if ( document.cookie && document.cookie.indexOf( 'cpmobile_switch_toggle' ) > -1 ) {
	// just switch
...
function cp_showhide_coms_toggle() {
	$wpt( '#commentlist' ).cpmobileFadeToggle( 350 );
	$wpt( 'img#com-arrow' ).toggleClass( 'com-arrow-down' );
	$wpt( 'h3#com-head' ).toggleClass( 'comhead-open' );
}

function docpmobileReady() {

	$wpt( '#headerbar-menu a' ).bind( touchStartOrClick, function( e ){
		$wpt( '#cpmobile-menu' ).cpmobileFadeToggle( 350 );
		$wpt( '#headerbar-menu a' ).toggleClass( 'open' );
	});
	// Toni +2.0
	//$wpt( 'a#searchopen, #cpmobile-search-inner a' ).bind( touchStartOrClick, function( e ){	
	$wpt( 'a#searchopen, #cpmobile-search-inner a' ).click( function(){ 
		$wpt( '#cpmobile-search' ).cpmobileFadeToggle( 350 );
	});

	$wpt( 'a#prowlopen' ).bind( touchStartOrClick, function( e ){	
		$wpt( '#prowl-message' ).cpmobileFadeToggle( 350 );
	});

	$wpt( 'a#wordtwitopen' ).bind( touchStartOrClick, function( e ){	
		$wpt( '#cpmobile-wordtwit' ).cpmobileFadeToggle( 350 );
	});

	$wpt( 'a#gigpressopen' ).bind( touchStartOrClick, function( e ){	
		$wpt( '#cpmobile-gigpress' ).cpmobileFadeToggle( 350 );
	});

	$wpt( 'a#loginopen, #cpmobile-login-inner a' ).bind( touchStartOrClick, function( e ){	
		$wpt( '#cpmobile-login' ).cpmobileFadeToggle(350);
	});

	$wpt( 'a#obook' ).bind( touchStartOrClick, function() {
		$wpt( '#bookmark-box' ).cpmobileFadeToggle(350);
	});
...
$wpt( document ).ready( function() { docpmobileReady(); } );
