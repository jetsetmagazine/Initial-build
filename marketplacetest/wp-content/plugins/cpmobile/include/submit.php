...
if ( isset( $_POST['submit'] ) ) {
	// let's rock and roll
	$nonce = $_POST['cpmobile-nonce'];
	if ( !wp_verify_nonce( $nonce, 'cpmobile-nonce' ) ) {
		_e( "Nonce Failure", "cpmobile" );
		die;
	}

	if ( !current_user_can( 'manage_options' ) ) {
		_e( "Security failure.  Please log in again.", "cpmobile" ); 
		die;
	}
...
//
	...
	if ( isset( $_POST['cpmobile-language'] ) ) {
		$a['cpmobile-language'] = $_POST['cpmobile-language'];
	} else {
		$a['cpmobile-language'] = "auto";	
	}
	...
	// Toni: + 2.0 Powered by is now off by default
	...
	$values = serialize($a);
	update_option('cp_iphone_pages', $values);
} elseif ( isset( $_POST['reset'] ) ) {
	update_option( 'cp_iphone_pages', false );
}
