...
/**/
if (!class_exists ("c_ws_widget__ad_codes_admin_notices"))
	{
		class c_ws_widget__ad_codes_admin_notices
			{
				/*
				...
				public static function enqueue_admin_notice ($notice = FALSE, $on_pages = FALSE, $error = FALSE, $time = FALSE, $dismiss = FALSE)
					{
						...
						do_action ("ws_widget__ad_codes_before_enqueue_admin_notice", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($notice && is_string ($notice)) /* If we have a valid string. */
							{
								$notices = (array)get_option ("ws_widget__ad_codes_notices");
								/**/
								array_push ($notices, array ("notice" => $notice, "on_pages" => $on_pages, "error" => $error, "time" => $time, "dismiss" => $dismiss));
								/**/
								eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action ("ws_widget__ad_codes_during_enqueue_admin_notice", get_defined_vars ());
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								update_option ("ws_widget__ad_codes_notices", c_ws_widget__ad_codes_utils_arrays::array_unique ($notices));
							}
						/**/
						do_action ("ws_widget__ad_codes_after_enqueue_admin_notice", get_defined_vars ());
						/**/
				...
				*/
				public static function display_admin_notice ($notice = FALSE, $error = FALSE, $dismiss = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_widget__ad_codes_before_display_admin_notice", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($notice && $error) /* Special format for errors. */
							{
								$notice .= ($dismiss) ? ' [ <a href="' . add_query_arg ("ws-widget--ad-codes-dismiss-admin-notice", urlencode (md5 ($notice)), $_SERVER["REQUEST_URI"]) . '">dismiss message</a> ]' : '';
								/**/
						...
						/**/
						do_action ("ws_widget__ad_codes_after_display_admin_notice", get_defined_vars ());
						/**/
						return; /* Return for uniformity. */
					}
				/*
				...
				public static function admin_notices ()
					{
						...
						/**/
						do_action ("ws_widget__ad_codes_before_admin_notices", get_defined_vars ());
						/**/
						if (is_admin () && is_array ($notices = get_option ("ws_widget__ad_codes_notices")) && !empty ($notices))
							{
												...
												if (!$page || "*" === $page || $pagenow === $page || $_GET["page"] === $page)
													{
														if (strtotime ("now") >= (int)$notice["time"]) /* Time to show it? */
															{
																eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																do_action ("ws_widget__ad_codes_during_admin_notices_before_display", get_defined_vars ());
																unset ($__refs, $__v); /* Unset defined __refs, __v. */
																/**/
																if (!$notice["dismiss"] || $_GET["ws-widget--ad-codes-dismiss-admin-notice"] === md5 ($notice["notice"]))
																	unset ($notices[$i]); /* Clear this administrative notice now? */
																/**/
																if (!$notice["dismiss"] || $_GET["ws-widget--ad-codes-dismiss-admin-notice"] !== md5 ($notice["notice"]))
																	c_ws_widget__ad_codes_admin_notices::display_admin_notice ($notice["notice"],$notice["error"],$notice["dismiss"]);
																/**/
																do_action ("ws_widget__ad_codes_during_admin_notices_after_display", get_defined_vars ());
															}
														/**/
								...
								/**/
								eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action ("ws_widget__ad_codes_during_admin_notices", get_defined_vars ());
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								update_option ("ws_widget__ad_codes_notices", $notices);
							}
						/**/
						do_action ("ws_widget__ad_codes_after_admin_notices", get_defined_vars ());
						/**/
						return; /* Return for uniformity. */
					}
			}
