...
if (!function_exists('cp_login_head')) {
    function cp_login_head() {
        if (is_user_logged_in()) :
			...
			?>
			<?php _e('Welcome,','appthemes'); ?> <strong><?php echo $current_user->user_login; ?></strong> [ <a href="<?php echo CP_DASHBOARD_URL ?>"><?php _e('My Dashboard','appthemes'); ?></a> | <a href="<?php echo $logout_url; ?>"><?php _e('Log out','appthemes'); ?></a> ]&nbsp;
		<?php else : ?>
...
if (!function_exists('cp_get_image_url')) {
	function cp_get_image_url() {
		...
		if ($images) {
			...
			$adthumbarray = wp_get_attachment_image_src($image->ID, 'small');
...
if (!function_exists('cp_get_image_url_raw')) {
    function cp_get_image_url_raw($post_id = '', $size = 'large', $class = '', $num = 1) {
        $images = get_posts(array('post_type' => 'attachment', 'numberposts' => $num, 'post_status' => null, 'post_parent' => $post_id, 'order' => 'ASC', 'orderby' => 'ID'));
        if ($images) {
            foreach ($images as $image) {
        ...
        if (in_array($result->field_name, $custom_field_keys) || ($result->field_name == 'post_content') || ($result->field_name == 'post_title') || ($result->field_name == 'tags_input')) :
			...
			case 'checkbox':
				...
				<li id="list_<?php esc_attr_e($result->field_name); ?>">
                    ...
                    <input type="hidden" name="<?php esc_attr_e($result->field_name); ?>" id="<?php esc_attr_e($result->field_name); ?>_value" value="<?php esc_attr_e($post_meta_val); ?>"  style="display:none;" />
					<div class="clr"></div>
				</li>
		...
		switch ($flag) :
			...
			case is_tax():
				...
				if(!empty($parents)):
					...
					foreach ($parents as $parent):
						...
						echo '<a href="'.$url.'#nav2">'.$item->name.'</a> ' . $delimiter . ' ';
					endforeach;
				endif;
			...
			case is_archive():
				if( !empty($_GET['sort']) && $_GET['sort'] == 'random')
				    echo __( "Random Ads ", 'apthemes' );
				else
				    echo __( "Latest Ads ", 'apthemes' );
			break;
...
//$output .= '<h3>'. __('Similar Items','appthemes') . '</h3>';
        ...
        if ($entries) {
            ...
            foreach ($entries as $post) {
                ...
                $output .= '<span class="list_ad_wrap_wide"><a class="list_ad_link_wide" href="'.get_permalink($post->object_id).'#nav2">'. $post->post_title. '</a><br/>';
...
if (!function_exists('cp_directory_cat_columns')) {
    function cp_directory_cat_columns($cols) {
        ...
        foreach($cats as $cat) {
            ...
            <ul>
                    <li class="maincat"><a href="<?php echo get_category_link($cat->term_id); ?>#nav2"><?php echo $cat->name; ?></a> (<?php echo $cat->category_count; ?>)</li>
...
function cp_renew_ad_listing ( $ad_id ) {
	...
	if ( $listfee == 0 )	{
		...
		wp_update_post( array('ID' => $ad_id, 'post_date' => date('m/d/Y H:i:s')) );
		return true;
	}
...
function get_pack_benefit($membership, $returnTotal = false) {
	...
	switch ( $membership->pack_type ) {
		...
		case 'required_static':
			if($returnTotal) return number_format($membership->pack_price*1);
			if((int)$membership->pack_price === 0) $benefitHTML .= __('Free Posting','appthemes');
...
function cp_fix_facebook_bug() {
	...
	if ( 'wp-login.php' == $pagenow && isset( $_GET['action'] ) && 'logout' == $_GET['action'] && isset( $_GET['amp;_wpnonce'] ) ) {
		wp_redirect( "/wp-login.php?action=logout&_wpnonce={$_GET['amp;_wpnonce']}" );
		exit;
	}
}
