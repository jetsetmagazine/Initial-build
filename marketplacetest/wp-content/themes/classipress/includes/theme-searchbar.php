			...
			<div class="searchblock">
				...
				<form action="<?php bloginfo('url'); ?>" method="get" id="searchform" class="form_search">
					...
					<div class="searchbutcat">
						...
						<button class="btn-topsearch" type="submit" tabindex="3" title="" id="go" value="search" name="sa"><?php _e('Search Ads', 'appthemes'); ?></button>

						<?php wp_dropdown_categories('show_option_all='.__('Select A Category', 'appthemes').'&hierarchical='.get_option('cp_cat_hierarchy').'&hide_empty='.get_option('cp_cat_hide_empty').'&depth='.get_option('cp_search_depth').'&show_count='.get_option('cp_cat_child_count').'&orderby='.get_option('cp_cat_orderby').'&title_li=&use_desc_for_title=1&tab_index=2&class=searchbar&taxonomy='.APP_TAX_CAT); ?>
