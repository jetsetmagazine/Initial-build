...
function cp_edit_ad_fields($results, $post_id) {
    ...
    foreach ( $results as $result ) :
        ...
        if ( in_array($result->field_name, $custom_field_keys) ) :
				...
				<tr>
					...
					<td>
						<div class="scrollbox">
							...
							<?php foreach ( $options as $option ) { ?>
								<li>
									<input type="checkbox" name="<?php echo $result->field_name; ?>[]" value="<?php echo trim($option); ?>" class="checkboxlist" <?php if(stristr( $post_meta_val, trim($option))) { echo 'checked="checked"'; } ?> />&nbsp;&nbsp;&nbsp;<?php echo trim($option); ?>
								</li> <!-- #checkbox -->		
							<?php } ?>	
...
function cp_save_meta_box($post_id) {
	...
	foreach ( $metafields as $name ) {
		...
		update_post_meta( $post_id, $name, esc_html($_POST["$name"]) );
	}	
