...
function displaySearch() {
	...
	var image_alt_Text = $(this).parent().text();
	$('#search_box').find('div').hide();
	$("form.category_search_form select:nth-child(1) option").attr('selected', false)
	$("form.category_search_form select:nth-child(1) option:contains('"+image_alt_Text+"')").attr('selected', true);
	$("form.search_form select option:nth-child(1)").attr('selected', true);
	$('#search_box input').val("Keyword");
	$('div[data-tag="'+image_alt+'"]').fadeIn("slow");	
};
$("div.market_nav img").click(displaySearch); 	
